...
function processFile (fname) {
    let text;
    try {
        text = fs.readFileSync(fname, { encoding: "utf8" });
    }
    catch (e) {
        console.error(e);
        return;
    ...
    let output;
    if (fname.endsWith(".html") || fname.endsWith(".vue")) {
        const styleTagAndContent = /(<style[^>]*>\s*)([\s\S]*?)(\s*<\/style>)/gi;
        output = text.replace(styleTagAndContent, (_, start, css, end) => {
            const lines = css.split(/\r?\n/g);
            const processed = processLines(lines).join("\n");
